<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-3">
            ...
            <h1 class="text-base font-semibold text-slate-900 dark:text-slate-100">{{ __('Edit Profile') }}</h1>
        ...
        <div class="space-y-4">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:shadow-black/20">
                <form method="post" action="{{ route('profile.update') }}" class="space-y-4">
                    ...
                    <div>
                        <label for="email" class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">{{ __('Email') }}</label>
                        <input
                            id="email"
                            name="email"
                            type="email"
            ...
            <div class="rounded-2xl border border-rose-200 bg-rose-50 p-4 shadow-sm dark:border-rose-900/80 dark:bg-rose-950/30 dark:shadow-black/20">
                <h2 class="text-sm font-semibold text-rose-800 dark:text-rose-300">{{ __('Delete Account') }}</h2>
                <p class="mt-1 text-xs text-rose-700 dark:text-rose-400">{{ __('This action is permanent.') }}</p>
                ...
                <form method="post" action="{{ route('profile.destroy') }}" class="mt-3 space-y-3" onsubmit="return confirm('Delete your account permanently?');">
                    ...
                    <div>
                        <label for="delete_password" class="mb-1 block text-sm font-medium text-rose-800 dark:text-rose-300">{{ __('Current Password') }}</label>
                        <input
                            id="delete_password"
                            name="password"
                            type="password"
                            ...
                            class="w-full rounded-xl border-rose-200 bg-white px-3.5 py-2.5 text-sm text-slate-700 shadow-sm focus:border-rose-400 focus:ring-rose-300 dark:border-rose-900 dark:bg-slate-900 dark:text-slate-100 dark:focus:border-rose-500 dark:focus:ring-rose-500"
                        >

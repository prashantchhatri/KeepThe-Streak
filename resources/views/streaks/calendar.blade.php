<x-app-layout>
    ...
    >
        <div x-show="toast.show" class="pointer-events-none fixed inset-x-0 bottom-6 z-50 flex justify-center px-4" x-cloak>
            <div class="rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow-lg" x-text="toast.message"></div>
        </div>

        <div class="space-y-4">
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div class="mb-4">
                    <label for="streak_switcher" class="mb-1 block text-xs font-medium text-slate-500">{{ __('Switch streak') }}</label>
                    <select
                        id="streak_switcher"
                ...
                <div class="mb-4 flex items-center gap-2">
                    <a
                        ...
                        @class([
                            ...
                            'border-slate-200 bg-white text-slate-700 hover:bg-slate-100 active:scale-95' => $canGoPrevious,
                            'pointer-events-none cursor-not-allowed border-slate-100 bg-slate-100 text-slate-300' => ! $canGoPrevious,
                        ])
                    >
                        &lt;
                    </a>
                    ...
                    <form method="GET" action="{{ route('streaks.calendar', $streak->id) }}" class="grid flex-1 grid-cols-2 gap-2">
                        <select
                            name="month"
                            class="w-full rounded-xl border-slate-200 px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-indigo-400 focus:ring-indigo-400"
                    ...
                    <a
                        ...
                        @class([
                            'inline-flex h-11 w-11 items-center justify-center rounded-xl border text-xl font-bold leading-none transition duration-200',
                            'border-slate-200 bg-white text-slate-700 hover:bg-slate-100 active:scale-95' => $canGoNext,
                            'pointer-events-none cursor-not-allowed border-slate-100 bg-slate-100 text-slate-300' => ! $canGoNext,
                        ])
                    >
                        &gt;
                    </a>
                ...
                <div class="mb-4 flex items-center justify-between gap-2">
                    <h3 class="text-base font-bold text-slate-900">{{ $monthLabel }}</h3>
                    <span class="text-xs text-slate-400">{{ __('Tap to update') }}</span>
                </div>
                ...
                <div class="mb-3 flex flex-wrap gap-2 text-xs text-gray-500">
                    ...
                    <span class="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-1">
                        <span class="h-2.5 w-2.5 rounded-sm bg-gray-300"></span>{{ __('Not marked') }}
                    </span>
                </div>
                            ...
                            @for ($day = 1; $day <= $daysInMonth; $day++)
                                ...
                                <button
                                    ...
                                    @class([
                                        ...
                                        'ring-2 ring-indigo-300 ring-offset-1' => $date === $todayDate,
                                    ])
                                >
                                    {{ $day }}
                                </button>
        ...
        <div
            x-show="editor.open"
            x-cloak
            class="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 px-4 pb-20 sm:items-center sm:pb-4"
            @click.self="closeEditor()"
        >
            <div class="w-full max-w-sm rounded-2xl bg-white p-4 shadow-xl">
                <h2 class="text-sm font-semibold text-slate-900">{{ __('Update Day') }}</h2>
                <p class="mt-1 text-xs text-slate-500" x-text="editor.dateLabel"></p>
                ...
                <button
                    type="button"
                    class="mt-3 inline-flex min-h-10 w-full items-center justify-center rounded-xl bg-slate-100 px-3 text-sm font-medium text-slate-700 transition duration-200 hover:bg-slate-200 active:scale-95"
                    @click="closeEditor()"
                >
                    {{ __('Cancel') }}
                </button>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('streakCalendar', (config) => ({
                ...
                dayClass(date, isFuture) {
                    if (isFuture) return 'bg-slate-100 text-slate-400 opacity-50';
